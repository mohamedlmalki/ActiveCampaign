...
router.post("/import/contact", async (req, res) => {
    ...
    try {
        ...
        console.error(`[OMNISEND] ❌ Import Failed for ${contact.email}:`, error.message);
        
        // Send a structured error back so the frontend job shows it clearly
        const errorDetails = error.response?.data || error.message;
        res.status(400).json({ error: "Import failed", details: errorDetails });
    }
});
