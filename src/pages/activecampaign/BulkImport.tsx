...
import { Upload, Play, Pause, Play as PlayIcon, StopCircle, FileJson } from "lucide-react";
...
export default function BulkImport() {
  ...
  const saveDraft = (updates: any) => {
      ...
      sessionStorage.setItem(`ac_draft_${activeAccount.id}`, JSON.stringify(currentState));
  };
  ...
  const handleStartImport = () => {
    ...
    const newJobId = addJob({
        ...
        data: contacts,
        batchSize: 1,
        apiEndpoint: '/api/activecampaign/import/contact',
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Import users into an ActiveCampaign list.</p>
        </div>
      </div>
            ...
            <div className="flex gap-2">
              {!isJobActive ? (
                ...
                <div className="flex gap-2 w-full">
                    <Button 
                        variant="outline" 
                        onClick={() => activeJob?.status === 'paused' ? resumeJob(activeJob.id) : pauseJob(activeJob!.id)} 
                        className="flex-1"
                    >
      ...
      <Dialog open={detailsModalOpen} onOpenChange={setDetailsModalOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>Raw response from ActiveCampaign.</DialogDescription>
            </DialogHeader>
