...
export default function SingleUserImport() {
  ...
  // --- AUTO-LOAD DRAFT (Benchmark Keys) ---
  useEffect(() => {
    if (activeAccount) {
      const savedList = localStorage.getItem(`draft_bm_single_list_${activeAccount.id}`);
      const savedEmail = localStorage.getItem(`draft_bm_single_email_${activeAccount.id}`);
      const savedFname = localStorage.getItem(`draft_bm_single_fname_${activeAccount.id}`);
      const savedLname = localStorage.getItem(`draft_bm_single_lname_${activeAccount.id}`);
  ...
  // --- AUTO-SAVE DRAFT (Benchmark Keys) ---
  useEffect(() => {
    if (activeAccount) {
      if(selectedList) localStorage.setItem(`draft_bm_single_list_${activeAccount.id}`, selectedList);
      localStorage.setItem(`draft_bm_single_email_${activeAccount.id}`, formData.email);
      localStorage.setItem(`draft_bm_single_fname_${activeAccount.id}`, formData.firstName);
      localStorage.setItem(`draft_bm_single_lname_${activeAccount.id}`, formData.lastName);
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Start Job (Engine handles Benchmark switch automatically)
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-semibold">Single User Import (Benchmark)</h1>
          <p className="text-muted-foreground">Add a new contact to a Benchmark Email list.</p>
        </div>
      </div>
                                        ...
                                        <TableRow key={result.index}>
                                            ...
                                            <TableCell>
                                                <Dialog>
                                                    ...
                                                    {/* Flexible Popup */}
                                                    <DialogContent className="max-w-[600px] max-h-[60vh] flex flex-col">
